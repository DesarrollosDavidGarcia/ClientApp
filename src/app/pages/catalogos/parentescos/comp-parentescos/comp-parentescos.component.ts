...
import { FormaParentescosComponent } from '../forma-parentescos/forma-parentescos.component';
import { Parentescos } from 'src/app/models/catalogos/parentescos';
import { UrlEnpoint } from 'src/app/models/utils/url-enpoint';


@Component({
  selector: 'app-comp-parentescos',
  templateUrl: './comp-parentescos.component.html',
  styleUrls: ['./comp-parentescos.component.scss']
})
export class CompParentescosComponent implements OnInit, OnDestroy {

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild('filtro', { static: false }) filtro: ElementRef;

  fuenteDatos: MatTableDataSource<Parentescos> = new MatTableDataSource<Parentescos>([]);
  columnasMostradas = [
    'codigo',
    'descripcion',
    'activo',
  ...
  async tabla(): Promise<void> {
    try {
      let resultado = await this.ctx.parentescos.getAll();
    if (resultado.data.length > 0) {
      this.fuenteDatos = new MatTableDataSource<Parentescos>(resultado.data);
    }else{
      this.fuenteDatos = new MatTableDataSource<Parentescos>([]);
    }
    this.fuenteDatos.paginator = this.paginator;
    this.fuenteDatos.sort = this.sort;
    } catch (error) {
  ...
  abrirForma(modelo?: Parentescos): void {
    let dato = modelo?.id ? modelo : null;
    let forma = this.ventana.open(FormaParentescosComponent, { data: dato, panelClass: 'form-container' });
  ...
  cambiarEstado(activo: boolean, modelo: Parentescos): void {
    if (activo) {
      this.activar(modelo);
    } else {
  ...
  async activar(modelo: Parentescos): Promise<void> {
    try {
      let resultado = await this.ctx.parentescos.active(modelo.id, modelo);
  ...
  async desactivar(modelo: Parentescos): Promise<void> {
    try {
      let resultado = await this.ctx.parentescos
  ...
  async eliminar(modelo: Parentescos): Promise<void> {
    ...
    try {
      ...
      forma.componentInstance.guardado.subscribe(async (respuesta: ResultVM<Mensaje>) => {
        ...
        {
          let resultado = await this.ctx.parentescos.delete(modelo.id);
  ...
  async importar() {
    try {
      ...
      model.path = "CatalogosSAC/ObtenerParentescosAsync";
      let respuestaSAC = await this.ctx.parentescos.getAllSAC(model);
      if (respuestaSAC.isSuccess) {
        let respuesta = await this.ctx.parentescos.addList(respuestaSAC.data);
        if (respuesta.isSuccess) {
          await this.tabla();
        }
      } else {
