...
@Component({
  selector: 'app-forma-grados-escolares',
  templateUrl: './forma-grados-escolares.component.html',
  styleUrls: ['./forma-grados-escolares.component.scss']
})
export class FormaGradosEscolaresComponent implements OnInit , OnDestroy {
  ...
  modelo: Menus;
  constructor(
    @Inject(MAT_DIALOG_DATA) public registro: GradoEscolaridad,
    private ventana: MatDialogRef<FormaGradosEscolaresComponent>,
  ...
  async guardar(): Promise<void> {
    let respuesta: ResultVM<GradoEscolaridad>;
    try {
      if (this.forma.valid) {
        const model = this.forma.value as GradoEscolaridad;

        if (this.registro?.id && this.registro.id !== 0) {
          respuesta = await this.ctx.gradoEscolaridad.update(this.registro.id, model);
        } else {
          respuesta = await this.ctx.gradoEscolaridad.add(model);
        }
