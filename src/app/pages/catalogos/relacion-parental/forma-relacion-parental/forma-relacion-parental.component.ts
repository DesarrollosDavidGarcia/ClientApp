...
@Component({
  selector: 'app-forma-relacion-parental',
  templateUrl: './forma-relacion-parental.component.html',
  styleUrls: ['./forma-relacion-parental.component.scss']
})
export class FormaRelacionParentalComponent implements OnInit , OnDestroy {
  ...
  modelo: Menus;
  constructor(
    @Inject(MAT_DIALOG_DATA) public registro: RelacionParental,
    private ventana: MatDialogRef<FormaRelacionParentalComponent>,
  ...
  async guardar(): Promise<void> {
    let respuesta: ResultVM<RelacionParental>;
    try {
      if (this.forma.valid) {
        const model = this.forma.value as RelacionParental;

        if (this.registro?.id && this.registro.id !== 0) {
          respuesta = await this.ctx.relacionParental.update(this.registro.id, model);
        } else {
          respuesta = await this.ctx.relacionParental.add(model);
        }
