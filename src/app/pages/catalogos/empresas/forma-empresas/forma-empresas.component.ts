...
@Component({
  selector: 'app-forma-empresas',
  templateUrl: './forma-empresas.component.html',
  styleUrls: ['./forma-empresas.component.scss']
})
export class FormaEmpresasComponent implements OnInit , OnDestroy {
  ...
  modelo: Empresas;
  constructor(
    @Inject(MAT_DIALOG_DATA) public registro: Empresas,
    private ventana: MatDialogRef<FormaEmpresasComponent>,
  ...
  async guardar(): Promise<void> {
    let respuesta: ResultVM<Empresas>;
    try {
      if (this.forma.valid) {
        const model = this.forma.value as Empresas;

        if (this.registro?.id && this.registro.id !== 0) {
          respuesta = await this.ctx.empresas.update(this.registro.id, model);
        } else {
          respuesta = await this.ctx.empresas.add(model);
        }
